...
class DocumentParser:
    def parse(self, file_path: str, filename: str) -> list[dict]:
        return list(self.iter_pages(file_path, filename))

    def iter_pages(self, file_path: str, filename: str):
        path = Path(file_path)
        ...
        try:
            if ext == ".pdf":
                yield from self._iter_pdf(file_path, filename)
            elif ext == ".docx":
                yield from self._parse_docx(file_path, filename)
            elif ext == ".pptx":
                yield from self._parse_pptx(file_path, filename)
            elif ext == ".csv":
                yield from self._parse_csv(file_path, filename)
            elif ext == ".txt":
                yield from self._parse_txt(file_path, filename)
            else:
    ...
    def _iter_pdf(self, file_path: str, filename: str):
        import fitz  # Load only the parser needed for this file type.
        with fitz.open(file_path) as document:
            for page_idx in range(len(document)):
                cleaned = self._clean_text(document[page_idx].get_text())
                if cleaned:
                    yield {
                        "text": cleaned,
    ...
    def _parse_csv(self, file_path: str, filename: str) -> list[dict]:
        rows_text = []
        # Standard-library CSV parsing avoids a pandas DataFrame and its imports.
        with open(file_path, "r", encoding="utf-8-sig", newline="") as stream:
            for row in csv.DictReader(stream):
                rows_text.append(", ".join(f"{col}: {val or ''}" for col, val in row.items()))
