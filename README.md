...
**How it works:** Uploaded files are parsed, chunked, embedded (BGE-small-en-v1.5 via FastEmbed), and stored as vectors in Qdrant, with raw files kept in Supabase Storage. User queries are embedded and matched against these vectors (scoped per user), and Llama 3.3 70B via Groq generates the final cited answer.
